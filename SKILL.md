...
- notify hook 已配置
- tmux 可用
- GitHub CLI `gh` 已登录并有目标仓库权限（需要创建 PR 时）
...
对 Git 项目中的代码变更，默认采用以下模型：

- `dev`：开发集成分支
- `release`：发布目标分支
- `feature/<task-id>`：单个任务的开发分支，从最新 `dev` 拉出

规则：

1. 不直接在 `dev` 上开发。
2. Codex 只在指定 feature branch / worktree 内开发、测试和提交。
3. OpenClaw 负责集成：测试通过后，将 feature 分支串行合并回 `dev`。
4. 合并回 `dev` 后必须再次运行项目测试；测试失败则让 Codex 修复，不创建 PR。
5. `dev` 测试通过后，push `dev`，再用 `gh` 创建或更新 `dev -> release` PR。
6. 同一项目允许多个 Codex 并行开发，但合并到 `dev` 必须加项目级锁并串行执行。
7. 遇到合并冲突、测试失败、branch protection、push/PR 权限问题时，停止发布步骤并汇报委托人或让 Codex 修复。
...
- 对 Git 项目中的代码变更，默认规划 `dev -> feature/<task-id> -> dev -> release PR`
- 如果同一项目已有任务在跑，必须使用独立 `git worktree` 和独立 feature 分支
...
- 明确任务边界（做什么、不做什么）
- 提供上下文（文件路径、技术栈、约束）
- 指定 Codex 应自行检查项目、完成修改、运行测试、提交 feature 分支变更、汇报结果
- 指定完成条件
- 复杂任务拆分步骤
...
后续 Codex 必须在该 feature worktree 中执行。非 Git 项目、只读分析任务或委托人明确指定分支策略时，可跳过此步骤。
...
1. 在 feature worktree 中确认测试通过。
2. 确认 feature 分支变更已提交；如 Codex 未提交，则检查 diff 后提交。
3. 获取项目级 `dev` 合并锁，确保同一项目只有一个任务正在合并。
4. 更新 `dev`：`git switch dev && git pull --ff-only origin dev`。
5. 合并 feature：`git merge --no-ff feature/<task-id>`。
6. 在 `dev` 上重新运行项目测试。
7. 测试通过后 push `dev`。
8. 检查是否已有 `dev -> release` 打开的 PR；没有则用 `gh pr create --base release --head dev` 创建，有则更新/评论现有 PR。
9. 释放项目级合并锁。

如果合并冲突或测试失败，停止 push/PR，让 Codex 在对应上下文修复后再重试。
...
## 工作流 D：多项目 / 多 Codex 并行编排

详见 `workflows/parallel_projects.md`。

核心原则：

1. OpenClaw 负责调度、分支、worktree、合并锁、测试门禁、push 和 PR。
2. Codex 只负责单个 feature worktree 内的开发、测试、提交和结果汇报。
3. 多项目可以并行；同一项目的多个 feature 可以并行开发，但合并到 `dev` 必须串行。
4. `dev` 测试通过后，才创建或更新 `dev -> release` PR。
